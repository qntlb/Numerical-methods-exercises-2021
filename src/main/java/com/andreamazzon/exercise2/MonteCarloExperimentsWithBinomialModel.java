 ...
 * an object of type BinomialModelSimulator as underlying. We suppose to compute
 * the Monte-Carlo price of the option different times, possibly simulating the
 * underlying with a different seed at any time. Indeed, simulating the
 * underlying with different seeds gives rise to different realizations of the
 * payoff, and then to different prices. Here we want to see the min and max
 * price as well as an histogram describing the distribution of the values of
 * the prices.
 *
...
public class MonteCarloExperimentsWithBinomialModel implements MonteCarloExperiments {
	...
	/*
	 * It will be initialized in the constructed with its own values for the
	 * maturity and for the underlying
	 */
	 ...
	 *                                   [minBin, maxBin] where all the prices we
	 *                                   get lie
	 * @param numberOfPriceComputations, the number of times we compute the price,
	...
	public HistogramData getHistogram(int numberOfBins, int numberOfPriceComputations) {
		...
		/*
		 * We call the setters of the container class. In this way, the fields of our
		 * object have now a value which can be get after calling this method, by the
