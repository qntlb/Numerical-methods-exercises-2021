...
/**
 * This class represents a random variable, seen as a bunch of possible
 * realizations. Not to be confused with what we have seen in RandomVariable and
 * RandomVariableInterface: in that case, an object represented a specific
 * distribution, with the possibility of getting the density, the cdf, or to
 * generate realizations of a random variable with such a distribution. In this
 * case, instead, we take a pure Monte-Carlo approach and we don't even care
 * about the specific distribution of the random variable: indeed, an object of
 * this class will represent a sample of realizations. The realizations are
 * stored as an array of doubles. In this way, we can compute the average and
 * the variance of the sample, as well as performing operations between
 * different random variables (by performing those operations between the vector
...
public class RandomVariableFromArray {
	 ...
	 *
	 * @param realizations, the array storing the realizations of the random
	 *                      variable
	 */
	 ...
	 * calling this method
	 *
	 * @return the realizations of the random variable, as an one-dimensional array
	 *         of doubles
	 */
	 ...
	 *
	 * @param realizationIndex, the int identifying the specific simulated
	 *                          realization
	 *
	...
	public RandomVariableFromArray add(RandomVariableFromArray randomVariable) {
		...
		/*
		 * Basically the scheme is: 1. Get the realizations wrapped by the two objects.
		 * 2. Perform the operation 3. Wrap again the result.
		 */
	...
	public RandomVariableFromArray add(double value) {
		...
		/*
		 * Basically the scheme is: 1. Get the realizations wrapped by the object. 2.
		 * Perform the operation 3. Wrap again the result.
		 */
		return new RandomVariableFromArray(realizationsSum);
	}
